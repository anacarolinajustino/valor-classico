"""
Conector Clássicos Premium.
Site: https://www.classicospremium.com.br
Motor: Custom jQuery
Estratégia: requests + BeautifulSoup
Seletores: div.item-car
"""
...
FONTE = "classicospremium"
BASE_URL = "https://www.classicospremium.com.br"
...
def buscar(marca: str, modelo: str, paginas: int = 2) -> list[Anuncio]:
    ...
    logger.info("[classicospremium] busca: %d anúncio(s)", len(anuncios))
    return anuncios
...
def coletar_completo(max_paginas: int = 100) -> tuple[list[Anuncio], dict]:
    ...
    logger.info("[classicospremium] coleta completa: %s", metricas)
...
def _requisitar(sessao: requests.Session, url: str) -> Optional[str]:
    for i in range(1, MAX_RETRIES + 1):
        try:
            ...
            logger.warning("[classicospremium] tentativa %d/%d %s: %s", i, MAX_RETRIES, url, exc)
            if i < MAX_RETRIES:
                time.sleep(BACKOFF)
    return None
