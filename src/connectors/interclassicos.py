"""
Conector InterClássicos.
Site: https://www.interclassicos.com.br
Motor: AutoCerto platform
Estratégia: requests + BeautifulSoup
Seletores: li.list-group-item
"""
...
FONTE = "interclassicos"
BASE_URL = "https://www.interclassicos.com.br"
...
def buscar(marca: str, modelo: str, paginas: int = 2) -> list[Anuncio]:
    ...
    logger.info("[interclassicos] busca: %d anúncio(s)", len(anuncios))
    return anuncios
...
def coletar_completo(max_paginas: int = 100) -> tuple[list[Anuncio], dict]:
    ...
    logger.info("[interclassicos] coleta completa: %s", metricas)
...
def _requisitar(sessao: requests.Session, url: str) -> Optional[str]:
    for i in range(1, MAX_RETRIES + 1):
        try:
            ...
            logger.warning("[interclassicos] tentativa %d/%d %s: %s", i, MAX_RETRIES, url, exc)
            if i < MAX_RETRIES:
                time.sleep(BACKOFF)
    return None
